...
/**
 * POST /api/cron/rounds/resolve
 * Resolve a specific round using current market price
 */
export async function POST(request: NextRequest) {
  try {
    const body = await request.json()
    const { marketId, roundId } = body
    ...
    const resolvedPrice = await fetchCurrentPrice(symbol)
...
/**
 * PUT /api/cron/rounds/resolve/auto
 * Auto-resolve all pending rounds that have ended
 */
